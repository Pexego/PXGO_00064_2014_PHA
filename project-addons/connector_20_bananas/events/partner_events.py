...
class PartnerExporter(Exporter):
    ...
    def update(self, binding_id, mode):
        ...
        vals = {
            ...
            "codcliente": str(partner.bananas_id),
            "codcomercial": str(
...
def delay_export_partner_write(session, model_name, record_id, vals):
    ...
    if vals.get("bananas_synchronized", False) and \
        ...
        unlink_partner_pricelist.delay(
            session, model_name, partner.commercial_partner_id.id, priority=1)
    ...
    if 'bananas_synchronized' not in vals and \
            (vals.get('property_product_pricelist') or
             'commercial_discount' in vals or 'financial_discount' in vals):
        unlink_partner_pricelist.delay(
            session, model_name, partner.id, priority=1)
        export_partner_pricelist.delay(

...
class ResPartner(models.Model):
    ...
    def _compute_has_delivery_address(self):
        for partner in self:
            if partner.is_company and partner.child_ids and \
    ...
    def create_bananas_id(self):
        return self.env['ir.sequence'].next_by_id(
            self.env.ref(
                'connector_20_bananas.sequence_partner_bananas_id').id)
    ...
    def create(self, vals):
        ...
        if partner.parent_id and partner.type == 'delivery' and \
                partner.parent_id.bananas_synchronized:
    ...
    def write(self, vals):
        for partner in self:
            ...
            if vals.get('parent_id') and vals.get(
                    'type', partner.type) == 'delivery' and \
                    partner.parent_id.bananas_synchronized and not \
                    vals.get('bananas_synchronized',
                             partner.bananas_synchronized):
                vals['bananas_synchronized'] = True
            elif vals.get('bananas_synchronized') and partner.is_company and \
                    partner.child_ids and \
                    partner.child_ids.filtered(lambda r: r.type == 'delivery'):
                childs = partner.child_ids.filtered(
                    lambda r: r.type == 'delivery')
                for child in childs:
                    child.write({'bananas_synchronized': True})
            if 'bananas_synchronized' in vals and not \
                    vals['bananas_synchronized'] and partner.is_company and \
                    partner.child_ids and \
                    partner.child_ids.filtered(lambda r: r.type == 'delivery'):
                childs = partner.child_ids.filtered(
                    lambda r: r.type == 'delivery')
    ...
    def check_custom_pricelist(self, commercial_discount, financial_discount):
        self.ensure_one()
        if commercial_discount or financial_discount and \

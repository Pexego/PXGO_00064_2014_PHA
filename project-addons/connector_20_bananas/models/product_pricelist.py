...
class ProductPricelist(models.Model):
    ...
    def update_partner_pricelist(self, product):
        for custom_pricelist in \
                self.custom_partner_pricelist_ids.filtered('active'):
            custom_pricelist.recompute_prices(product)

    def check_create_custom_pricelist(
            self, partner, commercial_discount=False,
        ...
        if not custom_pricelist:
            custom_pricelist = self.env[
                'product.pricelist.custom.partner'].search(
                    [('active', '=', False), ('partner_id', '=', partner.id),
        ...
        if not custom_pricelist:
            custom_pricelist = self.env['product.pricelist.custom.partner'].create({
...
class ProductPricelistItem(models.Model):
        ...
        for item in self:
            if item.product_id:
                item.bananas_price = item.price_version_id.pricelist_id.price_get(
                    item.product_id.id, 1)[item.price_version_id.pricelist_id.id]
            else:
                item.banans_price = 0
    ...
    def write(self, vals):
        ...
        for item in self:
            item.price_version_id.pricelist_id.partner.update_partner_pricelist(item.product_id)
        return res

...
ACTION_TYPES = [
    ...
    ('prod_x_morefree_y', _('Buy X more Y free'))
]
...
class PromotionsRules(osv.Model):
    ...
    _columns = {
        ...
        'shop' : fields.many2one('stock.warehouse', 'Warehouse', required=True),
        ...
        'sales_agents':fields.many2many(
                  'sale.agent',
                  'rule_agent_cat_rel',
                  'agent_id'
                  'rule_id',
                  string="Sales Agents",
                  help="Applicable to just this agent, if none partner selected, it applies to all"
                                              ),
    ...
    def check_primary_conditions(self, cursor, user,
        ...
        if promotion_rule.sales_agents:
            applicable_agents_ids = [agent.id for agent in promotion_rule.sales_agents]
            agents_order_ids = [
                        agent.id \
                            for agent in order.sale_agent_ids.agent_id
                                ]
            if not set(applicable_agents_ids).intersection(agents_order_ids):
                raise Exception("Not applicable to Sales Agents")
        if promotion_rule.coupon_code:
...
class PromotionsRulesActions(osv.Model):
    ...
    def action_prod_x_morefree_y(self, cursor, user,
                             action, order, context=None):
        """
        'Buy X get Y free:[Only for integers]'
        @param cursor: Database Cursor
    ...
    def validate(self, cursor, user, vals, context):
        ...
        if vals['action_type'] in ['prod_x_morefree_y', ]:
            try:
                code_1, code_2 = vals['product_code'].split(",")
                assert (type(eval(code_1)) == str)
                assert (type(eval(code_2)) == str)

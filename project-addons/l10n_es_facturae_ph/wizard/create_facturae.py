    ...
    def create_facturae_file(self):
        ...
        def _parties_facturae():
            ...
            def create_administrative_centres(address, code):
                ...
                if address.website:
                    texto += '<WebAddress>' + address.website[
                                              :60] + '</WebAddress>'
                if address.email:
        ...
        def _invoice_items():
            ...
            texto = ''
            texto += '<Items>'
            fil = self.env['facturae.invoice.lines']

            for line in invoice.invoice_line:
                texto += '<InvoiceLine>'

                fil = fil.search([('invoice_line_id', '=', line.id)])
                if fil.contract_reference and \
                        fil.contract_reference.strip() > '':
                    texto += '<IssuerContractReference>{}' \
                             '</IssuerContractReference>'.\
                        format(fil.contract_reference)
                if fil.transaction_reference and \
                        fil.transaction_reference.strip() > '':
                    texto += '<IssuerTransactionReference>{}' \
                             '</IssuerTransactionReference>'.\
                        format(fil.transaction_reference)

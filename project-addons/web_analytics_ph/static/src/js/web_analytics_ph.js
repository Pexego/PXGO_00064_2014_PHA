const GA4_property_ID = 'G-CB3GSJL7XM';
...
openerp.web_analytics_ph = function(instance) {
    /*
    *  The Web Analytics Module inserts the Google Analytics 4 JS Snippet
    ...
    *  web_client.do_push_state() method which is responsible of changing the Odoo current url
    */
    ...
    (function() {
        ...
        ga.src   = ('https:' === document.location.protocol ? 'https://www' : 'http://www') +
                   '.googletagmanager.com/gtag/js?id=' + GA4_property_ID;
        var s = document.getElementsByTagName('script')[0];
        s.parentNode.insertBefore(ga,s);
    })();

    instance.web_analytics_ph.Tracker = instance.web.Class.extend({
        /*
            ...
            instance.session.rpc("/web/webclient/version_info", {})
                .done(function(res) {
                    gtag('event', 'login', {
                        ...
                        'user_name': instance.session.username
                    });
        ...
        */
        on_state_pushed: function(state) {
            ...
            if (state.model && state.view_type) {
                // Track the page
                var url = instance.web_analytics_ph.generateUrl({'model': state.model, 'view_type': state.view_type});
                this._push_event({
        ...
        include_tracker: function() {
            ...
            instance.web.FormView.include({
                init: function(parent, dataset, view_id, options) {
                    ...
                    this.on('record_created', self, function(r) {
                        var url = instance.web_analytics_ph.generateUrl({'model': self.model, 'view_type': 'form'});
                        t._push_event({
                            'category': self.model,
                            'action': 'create',
                            'label': url,
            ...
            instance.web.ActionManager.include({
                ir_actions_client: function (action, options) {
                    var url = instance.web_analytics_ph.generateUrl({'action': action.tag});
                    t._push_event({
            ...
            instance.web.View.include({
                do_execute_action: function(action_data, dataset, record_id, on_closed) {
                    ...
                    var url = instance.web_analytics_ph.generateUrl({'model': category, 'view_type': this.view_type});
                    t._push_event({
                        'category': category,
                        'action': action,
                        'label': url,
            ...
            instance.web.CrashManager.include({
                show_error: function(error) {
                    ...
                    var url = instance.web_analytics_ph.generateUrl(options);
                    t._push_event({
    ...
    instance.web_analytics_ph.generateUrl = function(options) {
        var url = '';
    ...
    instance.web_analytics_ph.setupTracker = function(wc) {
    ...
    instance.web.Client.include({
        bind_events: function() {
            this._super.apply(this, arguments);
            this.tracker = new instance.web_analytics_ph.Tracker(this);
        },
    });

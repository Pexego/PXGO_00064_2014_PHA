...
class MrpProduction(models.Model):
            ...
            for move in production.move_lines:
                if move.restrict_lot_id.name != move.used_lot:
                    raise exceptions.Warning(_('Lot error'),
                                             _('The required lot %s and the received lot %s not mismatch') % (move.restrict_lot_id.name, move.used_lot))
 ...
 move quantity %s and served quantity %s don't match""") %
    ...
    def _make_production_consume_line(self, cr, uid, line, context=None):
        ...
        move_id = super(MrpProduction, self)._make_production_consume_line(cr, uid, line, context)
        self.pool.get('stock.move').write(cr, uid, move_id, {'workcenter_id': line.workcenter_id.id}, context)
        return move_id
...
class MrpWorkcenter(models.Model):
    ...
    bom_line_ids = fields.One2many('mrp.bom.line', 'workcenter_id', 'Bom lines')

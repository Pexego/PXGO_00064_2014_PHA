...
class StockMoveReturnOperations(models.Model):
    ...
    workcenter_id = fields.Many2one(string='Workcenter',
                                    related='move_id.workcenter_id',
                                    store=True)
            ...
            if move.move_dest_id and \
                ...
                for wkcenter_line in move.move_dest_id.raw_material_production_id.workcenter_lines:
                    ...
                    if not wkcenter_line.doc_submited:
                        if wkcenter_line.workcenter_id.name not in \
                                docs_no_submited:
                            docs_no_submited.append(
                                wkcenter_line.workcenter_id.name)
        if docs_no_submited:

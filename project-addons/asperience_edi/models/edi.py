...
try:
    ...
    def asperience_log(f):
        def called(*args, **kargs):
            try:
                msg = (
                    str(args[0].__class__.__name__)
                    + "."
                    + str(f.__name__)
                    + "("
                    + str([i for i in args if not isinstance(i, dict)][3:])
                    + ")"
                )
            except:
                try:
...
def unicode_csv_reader(unicode_csv_data, dialect=csv.excel, **kwargs):
    ...
    for row in csv_reader:
        yield [
            unicodedata.normalize("NFKD", cell.decode("utf-8"))
            .encode("ascii", "ignore")
            .decode("utf-8")
            for cell in row
        ]
...
class edi_edi(osv.osv):
    ...
    _columns = {
        ...
        "length_identifier": fields.function(
            _length,
            readonly=True,
            method=True,
            store=False,
        ...
        "end_line": fields.selection(
            [("simple", "Unix/Linux"), ("win", "Windows")],
            "End line",
            required=True,
            size=24,
        ),
        "float_separator": fields.selection(
            [(",", ","), (".", ".")], "Float Separator", required=True, size=5
        ),
        ...
        "path_in_move": fields.char(
            "Path In Move",
            size=128,
        ),
        "path_out": fields.char(
            "Path Out",
            size=128,
        ),
        ...
        "line_csv_ids": fields.one2many(
            "edi.edi.line.csv", "edi", "Line Structured CSV"
        ),
        ...
        "ftp_path_in_archive": fields.char(
            "Path In Archive",
            size=128,
        ),
    ...
    def _create_attachment(self, cr, uid, ids, file_path, context=False):
        ...
        cc = 0
        while self.pool.get("ir.attachment").search(
            cr,
            uid,
            [
                ("res_model", "=", data_attach["res_model"]),
                ("name", "=", data_attach["name"]),
                ("datas_fname", "=", data_attach["datas_fname"]),
            ],
        ):
    ...
    def _create_files(self, cr, uid, ids, data, context=False):
        ...
        for filename in data:
            ...
            for line in data[filename]:
                file.write(line)
            file.close()
            edi._create_attachment(
                file_path=context["path_out"] + filename, context=context
            )
            if edi.copy_out:
                _logger.debug("Export: %s " % (filename))
                ftp = FTP(edi.ftp_url_out, edi.ftp_login_out, edi.ftp_password_out)
                file = codecs.open(
                    str(context["path_out"]) + str(filename), "r", edi.charset
                )
        ...
        for filename in data:
            ...
            if edi.quotechar:
                csv_writer = UnicodeWriter(
                    file,
                    encoding=edi.charset,
                    delimiter=str(edi.delimiter[0]),
                    quotechar=str(edi.quotechar),
                    quoting=csv.QUOTE_NONNUMERIC,
                )
            else:
                csv_writer = UnicodeWriter(
                    file, encoding=edi.charset, delimiter=str(edi.delimiter[0])
                )
        ...
        if edi.copy_in:
            ...
            for file_name in list:
                if not os.path.exists(path + file_name):
                    _logger.debug(
                        "RETR : %s, DEST : %s" % (file_name, path + file_name)
                    )
                    ftp.retrbinary(
                        "RETR " + file_name, open(path + file_name, "wb").write
                    )
            try:
    ...
    def _parse_line_csv_struct(self, cr, uid, ids, line, structs, context=False):
        ...
        if identifier not in structs:
            raise Exception(
                _("%s not found in CSV structured structure") % (identifier,)
            )
        struct = structs[identifier]
        ...
        for field in struct:
            ...
            if len(line) > struct[field]["sequence"]:
                result[struct[field]["name"]] = parseFunc(
                    line[struct[field]["sequence"]],
                    structs["<<>>"],
                    struct[field]["type"],
                    struct[field]["cast"],
                )
    ...
    def import_edi_thread(self, cr, uid, ids, context=False):
        ...
        thread_ptr = threading.Thread(
            target=self.import_edi, args=(cr2, uid, ids, context)
        )
        thread_ptr.start()
        return {}
            ...
            try:
                ...
                for filename in edi._get_files(edi.path_in, edi.file_template):
                    ...
                    try:
                        ...
                        shutil.move(
                            edi.path_in + os.path.basename(filename),
                            edi.path_in + "error/" + os.path.basename(filename),
                        )
                        raise e
                    if edi.path_in_move:
                        ...
                        shutil.move(
                            edi.path_in + os.path.basename(filename),
                            edi.path_in_move + os.path.basename(filename),
                        )
                    _logger.info("End Import file : %s" % (filename))
                    self.pool.get("edi.edi.result").create(
                        cr,
                        uid,
                        {
                            "name": "file_import_edi_ok",
                            "value": filename,
                            "edi": edi.id,
                        },
                    )
                ...
                if info:
                    self.pool.get("edi.edi.result").create(
                        cr,
                        uid,
                        {"name": "info_import_edi_nok", "value": info, "edi": edi.id},
                    )
                cr.commit()
                exception = True
        if not exception:
    ...
    def export_edi_thread(self, cr, uid, ids, context=False):
        ...
        cr2 = pooler.get_db(cr.dbname).cursor(serialized=False)
        thread_ptr = threading.Thread(
            target=self.export_edi, args=(cr2, uid, ids, context)
        )
        thread_ptr.start()
        return {}
            ...
            try:
                ...
                if info:
                    self.pool.get("edi.edi.result").create(
                        cr,
                        uid,
                        {"name": "info_export_edi_nok", "value": info, "edi": edi.id},
                    )
                cr.commit()
                exception = True
        if not exception:
    ...
    def import_csv_thread(self, cr, uid, ids, context=False):
        ...
        cr2 = pooler.get_db(cr.dbname).cursor(serialized=False)
        thread_ptr = threading.Thread(
            target=self.import_csv, args=(cr2, uid, ids, context)
        )
        thread_ptr.start()
        return {}
            ...
            try:
                ...
                if info:
                    self.pool.get("edi.edi.result").create(
                        cr,
                        uid,
                        {"name": "info_import_csv_nok", "value": info, "edi": edi.id},
                    )
                cr.commit()
                exception = True
        if not exception:
    ...
    def export_csv_thread(self, cr, uid, ids, context=False):
        ...
        cr2 = pooler.get_db(cr.dbname).cursor(serialized=False)
        thread_ptr = threading.Thread(
            target=self.export_csv, args=(cr2, uid, ids, context)
        )
        thread_ptr.start()
        return {}
            ...
            try:
                ...
                if info:
                    self.pool.get("edi.edi.result").create(
                        cr,
                        uid,
                        {"name": "info_export_csv_nok", "value": info, "edi": edi.id},
                    )
                cr.commit()
                exception = True
        if not exception:
    ...
    def import_xml_thread(self, cr, uid, ids, context=False):
        ...
        cr2 = pooler.get_db(cr.dbname).cursor(serialized=False)
        thread_ptr = threading.Thread(
            target=self.import_xml, args=(cr2, uid, ids, context)
        )
        thread_ptr.start()
        return {}
            ...
            try:
                ...
                if info:
                    self.pool.get("edi.edi.result").create(
                        cr,
                        uid,
                        {"name": "info_import_xml_nok", "value": info, "edi": edi.id},
                    )
                cr.commit()
                exception = True
        if not exception:
    ...
    def export_xml_thread(self, cr, uid, ids, context=False):
        cr2 = pooler.get_db(cr.dbname).cursor(serialized=False)
        thread_ptr = threading.Thread(
            target=self.export_xml, args=(cr2, uid, ids, context)
        )
        thread_ptr.start()
        return {}
            ...
            try:
                ...
                if info:
                    self.pool.get("edi.edi.result").create(
                        cr,
                        uid,
                        {"name": "info_export_xml_nok", "value": info, "edi": edi.id},
                    )
                cr.commit()
                exception = True
        if not exception:
    ...
    def import_csv_struct_thread(self, cr, uid, ids, context=False):
        ...
        thread_ptr = threading.Thread(
            target=self.import_csv_struct, args=(cr, uid, ids, context)
        )
        thread_ptr.start()
        return {}
                            ...
                            for line in file_csv:
                                ...
                                if line:
                                    data[filename].append(
                                        edi._parse_line_csv_struct(line, structs)
                                    )
                            file.close()
                            edi.eval_in = edi.eval_in.replace(chr(13), "\n")
    ...
    def _get_log(self, cr, uid, ids, context=False):
        ...
        if not edi["force"]:
            cr.execute(
                "select object_id from edi_edi_log where edi = %s" % str(context["edi"])
            )
            res = cr.dictfetchall()
            for log in res:
...
class edi_edi_log(osv.osv):
    ...
    _columns = {
        ...
        "type": fields.selection(
            [("export", "export"), ("import", "import")], "Type", required=True, size=24
        ),
...
class edi_edi_line_edi(osv.osv):
    ...
    def check_name(self, cr, uid, ids):
        for line in self.browse(cr, uid, ids):
            if (
                len(line.name) != int(line.edi.length_identifier)
                and int(line.edi.length_identifier) != 0
            ):
                return False
        return True
    ...
    _columns = {
        ...
        "dict": fields.function(
            _dict_fields_ids,
            readonly=False,
            method=True,
            store=False,
            string="Dict to put in eval",
            type="text",
        ),
    }
    _defaults = {
...
class edi_edi_line_edi_field(osv.osv):
    ...
    def check_start_stop(self, cr, uid, ids):
        for field in self.browse(cr, uid, ids):
            ...
            for next in field.line.fields_ids:
                if (
                    field.start >= next.start
                    and field.start <= next.stop
                    and field.id != next.id
                ):
                    return False
                elif (
                    field.stop >= next.start
                    and field.stop <= next.stop
                    and field.id != next.id
                ):
                    return False
        return True
    ...
    _columns = {
        ...
        "align": fields.selection(
            [("right", "right"), ("left", "left"), ("zero", "zero")],
            "Align",
            required=True,
            size=24,
        ),
    ...
    _constraints = [
        (check_start_stop, "Invalid start stop", ["start", "stop"]),
        (check_name, "The name must be unique !", ["name"]),
    ]
...
class edi_edi_csv_field(osv.osv):
    ...
    _constraints = [
        (check_name, "The name must be unique !", ["name"]),
        (check_sequence, "The sequence must be unique !", ["sequence"]),
    ]

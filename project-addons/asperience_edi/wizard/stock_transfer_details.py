        ...
        op_sscc = self.env["stock.pack.operation.sscc"].create(
            {
                "name": sscc,
                "type": type,
                "operation_ids": [(6, 0, [operation.id])],
                "parent": parent,
            }
        )
        ...
        for packop in self.picking_id.pack_operation_ids:
            ...
            if packop.complete:
                ...
                for i in range(packop.complete):
                    counter, op_sscc = self.make_sscc(
                        self.picking_id, packop, counter, "2", parent=parent
                    )
            if packop.package != 0:
                if packop.package not in package.keys():
                    ...
                    if packop.palet != 0:
                        parent = palet[packop.palet].id
                    counter, op_sscc = self.make_sscc(
                        self.picking_id, packop, counter, "3", parent=parent
                    )

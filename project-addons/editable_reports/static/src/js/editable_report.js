...
if (!imprimiendoEnPhantomJS) {
    ...
    script = script.replace('editable_report.js', 'phantom_incompatible_code.js');
        ...
        if (typeof model !== typeof undefined && model !== false) {
            ...
            obj.call('search', [[['name', '=', text]]]).then(function(result) {
                ...
                if (result.length === 0) {
                    ...
                    event.target.setAttribute('editable_report_value', result);
...
function preparaDate() {
    var inputsDate = $('.report_row input[type="date"]');
...
function dameFechaHoraServidor(formato) {
    var context = {lang: 'es_ES', tz: 'Europe/Madrid'},
        obj = new openerp.web.Model('editable.report', context),
        method = '';
    switch (formato) {
        case 'fecha':
            fechaServidor = false;
...
function preparaInputs() {
    ...
    if (imprimiendoEnPhantomJS) {
        $('.report_row input[type="number"], ' +
          '.report_row input[type="time"], ' +
          '.report_row input[type="datetime-local"]').each(
            function() {
                $(this).prop('type', 'text');
            }
        );
...
$(function () {
    ...
    $('#all_data').find('.report_form').each(function() {
        var record = Number($(this).attr('record'));
        var model = $(this).attr('model');
        var context = {lang: 'es_ES', tz: 'Europe/Madrid'};
        $(this).find('.report_field').each(function() {
                    ...
                    for (var i = 0; i<columns.length; i++) {
                        ...
                        if (key === 'id') {
                            ...
                            if (columns_options && columns_options[key] == 'hidden') {
                                ...
                                table_columns.push({
                                    name: key,
                                    display: fields[key].string,
                                    type: JQUERY_UI_TYPES[fields[key].type],
                                    ctrlProp: ctrlProp,
                        ...
                        if (response[field_to_represent].length > 0) {
                            ...
                            view_model.call('search_read', [read_filter, columns], {order: order_by, context: context}).then(function(rows_data) {
                                ...
                                self.appendGrid({
                                    ...
                                    initData: initData,
                                    customGridButtons:{
                                        append: $('<button/>').text('Insertar').get(0),
                                    },
...
function send_form_server() {
     ...
     */
    var write_vals = {};
    let reNotacionCientifica = /[0-9][eE][-+][0-9]+/;
            ...
            $(this).find('form').each(function(){
                ...
                $(this).find('.report_field').each(function() {
                    ...
                    for (var i = 0; i< elements.length; i++) {
                        if (elements[i].search(reNotacionCientifica) == -1) {
                        ...
                        if (compare && compare.length && field_name == compare[0]) {
                            ...
                            if ($('input[name="' + compare_name + '"]').length) {
                                ...
                                if (vals[1] != to_compare_val) {
                                    ...
                                    throw new Error('¡Algo fue muy mal!');
                                }
                            }
                        }
                    };
...
$.fn.val = function(value) {
    ...
    if (!arguments.length) {
        if (elem) {
            ...
            var editable_report_value = this.attr('editable_report_value');
            if (typeof editable_report_value !== typeof undefined && editable_report_value !== false) {
                ret = editable_report_value;
            } else {
                ret = elem.value;
            }
